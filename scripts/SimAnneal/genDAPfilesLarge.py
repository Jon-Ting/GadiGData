...
npCnt = 3828  # 2868 for RAL
eleComb = 'AuPt'
sourceDir = '/g/data/q27/jt5911/PostSim/CSAu60+'
...
for NPtarball in os.listdir(sourceDir):
    if isdir(f"{sourceDir}/{NPtarball}"): continue  # Skip directories
    NPdir = NPtarball[:-7]
    ...
    if not exists(f"{NPdirPath}/{doneFile}"):
        with tarfile.open(f"{sourceDir}/{NPtarball}", 'r') as f:
            f.extract(f"{NPdir}/{NPdir}S2.zip", sourceDir)
            f.extract(f"{NPdir}/{NPdir}S2.log", sourceDir)
        # print("    Extracted Stage 2 zip file...")  # For debugging
        with ZipFile(f"{NPdirPath}/{NPdir}S2.zip", 'r') as f: f.extractall(NPdirPath)
        # print("    Extracted Stage 2 files...")  # For debugging
    else:
        # print("    Done!")  # For debugging
        npCnt += 1
        continue

    # Copy renamed xyz file to targeted directory
    ...
    for NPconf in os.listdir(f"{NPdirPath}/{NPdir}S2"): 
        ...
        # shutil.copy(oriFilePath, f"{confDir}/{confID}.xyz")  # Superceded by lines above
    ...
    os.remove(f"{NPdirPath}/{NPdir}S2.zip")
    os.remove(f"{NPdirPath}/{NPdir}S2.log")
