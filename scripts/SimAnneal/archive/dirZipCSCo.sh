...
declare -a ELE1_ARR=('Co')
declare -a SIZE1_ARR=(60)
                ...
                for ((m=0;m<${#ELE2_ARR[@]};m++)); do
                    ...
                    for ((n=0;n<${#SIZE2_ARR[@]};n++)); do
                        ...
                        for ((o=0;o<${#SHAPE2_ARR[@]};o++)); do
                            ...
                            #fi
                            if grep -q $ele1Type$size1$shape1$ele2Type$size2$shape2${bnpType} zipped$ele1Type.txt; then continue; fi
                            zip -6 -li -r $bnpType$ele1Type.zip $bnpType/$ele1Type$size1$shape1$ele2Type$size2$shape2* -u -x *log.lammps
                            echo $ele1Type$size1$shape1$ele2Type$size2$shape2${bnpType} >> zipped$ele1Type.txt
                        done
                    done
                done
            done
